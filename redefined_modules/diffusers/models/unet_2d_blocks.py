...
from aimet_torch.elementwise_ops import Concat
...
class AttnSkipDownBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, skip_sample=None):
        ...
        if self.downsamplers is not None:
            ...
            hidden_states = self.add_1(self.skip_conv(skip_sample), hidden_states)
...
class AttnUpBlock2D(nn.Module):
    def __init__(
        ...
        self.cats = nn.ModuleList([Concat(1) for _ in range(len(self.resnets))])
    ...
    def forward(self, hidden_states, res_hidden_states_tuple, temb=None):
        ...
        for i, (resnet, attn) in enumerate(zip(self.resnets, self.attentions)):
            ...
            hidden_states = self.cats[i](hidden_states, res_hidden_states)
...
class AttnSkipUpBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, res_hidden_states_tuple, temb=None, skip_sample=None):
        ...
        if self.resnet_up is not None:
            ...
            skip_sample = self.add_1(skip_sample, skip_sample_states)
...
class SkipUpBlock2D(nn.Module):
    def __init__(
        ...
        self.add_1 = elementwise_ops.Add()
        self.cats = nn.ModuleList([Concat() for _ in range(len(self.resnets))])
