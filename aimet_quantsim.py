...
def export_all_models(te_sim, unet_sim, vae_sim, tokens, embeddings, latents):
    ...
    dummy_input = tokens[0][0].to(te_sim.model.device)
    export_quantsim_model(te_sim, '_exports_/text_encoder_onnx', dummy_input, 'text_encoder',
                          input_names=['input_1'], output_names=['output_1'])

    print("Exporting UNET ----")
    dummy_input = unet_dummy_input(unet_sim.model, embeddings)
    export_quantsim_model(unet_sim, '_exports_/unet_onnx', dummy_input, 'unet',
                          use_external_data_format=True,
                          input_names=['input_1', 'input_2', 'input_3'], output_names=['output_1'])

    print("Exporting VAE ----")
    dummy_input = latents[0]
    export_quantsim_model(vae_sim, '_exports_/vae_decoder_onnx', dummy_input, 'vae_decoder',
                            input_names=['input_1'], output_names=['output_1'])


def apply_adaround_te(fp_model, adaround_data, config):
    OUTPUT_PATH = './adaround_model'
    ...
    def _dummy_fw(model, train_data):
        return model(train_data[0]) # only adapt adaround for conditional inputs
    ...
    # save
    torch.save(ada_model.state_dict(), os.path.join(OUTPUT_PATH, "state_dict.pt"))
    ...
    quant_sim.set_and_freeze_param_encodings(encoding_path=os.path.join(OUTPUT_PATH, "parameter.encodings"))
    return quant_sim
...
def unet_dummy_input(unet_model, embeddings):
    ...
    dummy_latents = torch.randn((batch_size, unet_model.in_channels, h // 8, w // 8)).to(unet_model.device)
    time_emb_dim = unet_model.config["block_out_channels"][0] * 4
    dummy_time_emb = torch.randn((batch_size, time_emb_dim)).to(unet_model.device)
